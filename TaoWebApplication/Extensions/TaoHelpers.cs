        ...
        public static MvcHtmlString CreateActualControl<TModel>(this HtmlHelper<TModel> helper, IFieldList model, int i, ModelStateDictionary modelState = null)
        {
            ...
            if (!model.Fields[i].IsEditable || model.Fields[i].IsCaculated)
            {
                switch (model.Fields[i].TypeName)
                {
                    ...
                    case "numeric":
                        return helper.Label(model.Fields[i].DecimalValue.HasValue ? model.Fields[i].DecimalValue.Value.ToString("#,#;", CultureInfo.GetCultureInfoByIetfLanguageTag("HU-hu")) : "", new { @class = "TaoControl", @style = $"width:100%; background-color:{@color}; text-align:right;  border:thick;" });
                    case "date":
        ...
        public static MvcHtmlString CreateActualControlForTable<TModel>(this HtmlHelper<TModel> helper, ITableDescriptor model, int i, int j, int tableIndex, ModelStateDictionary modelState = null)
        {
            ...
            if (!model.TableDescriptors[tableIndex].FieldValues[i][j].IsEditable || model.TableDescriptors[tableIndex].FieldValues[i][j].IsCaculated)
            {
                switch (model.TableDescriptors[tableIndex].FieldValues[i][j].TypeName)
                {
                    ...
                    case "numeric":
                        return helper.Label(model.TableDescriptors[tableIndex].FieldValues[i][j].DecimalValue.HasValue ? model.TableDescriptors[tableIndex].FieldValues[i][j].DecimalValue.Value.ToString("#,#;", CultureInfo.GetCultureInfoByIetfLanguageTag("HU-hu")) : "", new { @class = "TaoControl", @style = $"width:100%; background-color:{@color}; text-align:right;  border:thick; font-weight: normal;" });
                    case "date":
